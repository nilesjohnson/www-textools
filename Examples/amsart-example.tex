...
\newcommand{\mylabel}[1]{\desclabel{#1}{#1}}
\newcommand{\myref}[1]{\OLDref{#1}\showdesc{#1}\writeref{#1}}
...
\renewcommand*\label[1]{%
	\barelabel{#1}%
        \addtostream{Xrefvertices}{#1,"(\getrefnumber{#1})","Equation","#1",\getpagerefnumber{#1}}
	\ifmeasuring@
	\else
		%\global\@namedef{www@label@store@description@#1}{#1}%
		\ifcsname wwwnotes@isequation\endcsname
			\addtostream{Xrefvertices}{#1,"(\getrefnumber{#1})","Equation","#1",\getpagerefnumber{#1}}%
		\else
			\addtostream{Xrefvertices}{#1,"\getrefnumber{#1}","Item","#1",\getpagerefnumber{#1}}%
	\fi\fi} 
...
\end{lem}
Then we can apply \ref{eq001} and \ref{someprop} to get the final answer.
Here ends the proof.
\end{pfof}
